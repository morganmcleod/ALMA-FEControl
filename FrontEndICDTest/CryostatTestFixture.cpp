...
void CryostatTestFixture::testSET_CRYOSTAT_RESET_COLD_HEAD_HOURS() {
    ...
    dataLength_m = 1;
    data_m[0] = 0;
    command(coldHeadHoursReset_RCA, "SET_CRYOSTAT_RESET_COLD_HEAD_HOURS(0)", &details, true);

    // send the command to reset the cold head hours and check for no errors:
    resetAmbVars();
    dataLength_m = 1;
    data_m[0] = 1;
    command(coldHeadHoursReset_RCA, "SET_CRYOSTAT_RESET_COLD_HEAD_HOURS(1)", &details, true, 300);
